...
pub async fn upload_aws(
    app_state: &AppState,
    file_meta_data: &FileToInsert,
    data: &Bytes,
...
pub async fn delete_from_s3(delete_token: String, bucket_name: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
    let config = aws_config::load_defaults(aws_config::BehaviorVersion::latest()).await;
    let client = aws_sdk_s3::Client::new(&config);
...
pub async fn get_from_s3(
    bucket_name: String,
    file_key: String,
) -> Result<ByteStream, Box<dyn std::error::Error + Send + Sync>> {
    let config = aws_config::load_defaults(aws_config::BehaviorVersion::latest()).await;
    let client = aws_sdk_s3::Client::new(&config);
    let file = client
        .get_object()
