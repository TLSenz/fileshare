...
use crate::model::{ConversionError, DeleteError, FileToInsert, UploadOptions};
...
pub async fn create_link(pool: &PgPool, file: &FileToInsert) -> Result<String, ConversionError> {
    ...
    pub async fn delete_file_s3(pool: &PgPool, delete_token: &str, bucket_name: &str, file_key: &str) -> Result<String, DeleteError> {
    let does_file_exist = validate_delete_token(pool, delete_token).await;
        match does_file_exist {
            Ok(true) => {
                ...
                match delete_from_s3(file_key.to_string(), bucket_name).await.map_err(|_| DeleteError::DeletionFailed(StatusCode::BAD_GATEWAY))?;
            },
            Ok(false) => {
